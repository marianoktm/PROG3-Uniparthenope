...
import Server.Operations.OpCommands.UserLoginOperation;
...
/**
 * A CHAIN OF RESPONSIBILITY handler to handle USER_LOGIN request.
 */
public class LoginHandler extends OperationChain {
    private final RequestCode requestCode = RequestCode.USER_LOGIN;
    ...
    public Packet perform(Socket socket, Packet packet) throws SessionException, SQLException, InvalidTwitterOpException, BanException {
        if (canHandle(packet, requestCode))
            return (new UserLoginOperation(socket, packet)).execute();
        else if (next != null)
